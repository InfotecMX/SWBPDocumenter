...
public class DocumentTemplateResource extends GenericResource {
    ...
    public final static String VIEW_DOC = "vdoc";
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (isAdminSite) {
            ...
            SWBResourceURL url = paramRequest.getRenderUrl().setCallMethod(SWBResourceURL.Call_DIRECT).setMode(VIEW_DOC);
            url.setParameter("suri", suri);
            out.println("<iframe src=\"" + url + "\" style=\"width:100%; height:100%;\" frameborder=\"0\"></iframe>");
        } else {
//            String path = SWBPortal.getContextPath() + "/swbadmin/jsp/process/financiera/documentTemplate.jsp";
            SemanticObject semObj = SemanticObject.createSemanticObject(suri);
            if (semObj != null) {
                ProcessSite model = (ProcessSite) semObj.createGenericInstance();
                String path = "/work/models/" + model.getId() + "/jsp/documentation/documentTemplate.jsp";
...
//                        }
    ...
    public void doViewDoc(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (semObj != null) {
            ...
            try {
                ...
                log.error("Error on doViewDoc, " + path + ", " + ex.getMessage());
            }
        }
    }
    ...
    public void doAddDT(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (semObj != null) {
            ProcessSite model = (ProcessSite) semObj.createGenericInstance();
            String path = "/work/models/" + model.getId() + "/jsp/documentation/documentTemplateEdit.jsp";
            request.setAttribute("paramRequest", paramRequest);
            request.setAttribute("uridt", "");
            request.setAttribute("suri", request.getParameter("suri") != null ? request.getParameter("suri") : "");
            RequestDispatcher rd = request.getRequestDispatcher(path);
            try {
                rd.include(request, response);
            } catch (ServletException ex) {
    ...
    public void doAddDS(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (semObj != null) {
            ProcessSite model = (ProcessSite) semObj.createGenericInstance();
            String path = "/work/models/" + model.getId() + "/jsp/documentation/documentTemplateEditSection.jsp";
            request.setAttribute("paramRequest", paramRequest);
            request.setAttribute("suri", suri);
            request.setAttribute("uridt", request.getParameter("uridt"));
    ...
    public void doViewProperties(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (semObj != null) {
            ProcessSite model = (ProcessSite) semObj.createGenericInstance();
            String path = "/work/models/" + model.getId() + "/jsp/documentation/documentTemplateProperties.jsp";
            request.setAttribute("paramRequest", paramRequest);
            request.setAttribute("suri", suri);
    ...
    public void doViewLog(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        if (semObj != null) {
            ProcessSite model = (ProcessSite) semObj.createGenericInstance();
            String path = "/work/models/" + model.getId() + "/jsp/documentation/logView.jsp";
            request.setAttribute("paramRequest", paramRequest);
            request.setAttribute("suri", suri);
            request.setAttribute("uridt", request.getParameter("uridt"));
            RequestDispatcher rd = request.getRequestDispatcher(path);
    ...
    private List<DocumentTemplate> listTemplates(HttpServletRequest request, SWBParamRequest paramRequest) {
        ...
        WebSite model = (WebSite) SWBPlatform.getSemanticMgr().getOntology().getGenericObject(request.getParameter("suri"));
        if (model != null) {
            String lang = "es";
